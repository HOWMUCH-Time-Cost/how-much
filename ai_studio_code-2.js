...
function processNode(textNode) {
  ...
  const newText = originalText.replace(PRICE_REGEX, (match) => {
    try {
      ...
      // Handle Decimal/Thousand separators based on currency
      let priceValue = 0;
      if (detectedCurrency === 'BRL' || detectedCurrency === 'EUR') {
        // Swap dots and commas for JS parsing (1.000,00 -> 1000.00)
        cleanString = cleanString.replace(/\./g, '').replace(',', '.');
      } else {
        // USD (1,000.00 -> 1000.00)
      ...
      const dailyWageInUSD = salaryInUSD / 30;
      ...
      let daysString = "";
      if (daysCost < 0.1) {
        // If less than a tenth of a day, show hours (assuming 8h work day)
